...
def BrowserSelection(browser):
    ...
    try:
        if "Chrome" in browser:
            ...
            CoreFrameWork.CommonUtil.ExecLog(sModuleInfo, "Started Chrome Browser", 1)
            return "PASSED"
        elif browser == 'Firefox':
            sBrowser = webdriver.Firefox()
            CoreFrameWork.CommonUtil.ExecLog(sModuleInfo, "Started Firefox Browser", 1)
            print "Started Firefox Browser"
            return "PASSED"
        elif "IE" in browser:
            sBrowser = webdriver.Ie()
            CoreFrameWork.CommonUtil.ExecLog(sModuleInfo, "Started Internet Explorer Browser", 1)
            print "Started Internet Explorer Browser"
            return "PASSED"
        else:
            print "You did not select a valid browser: %s" % browser
            CoreFrameWork.CommonUtil.ExecLog(sModuleInfo, "You did not select a valid browser: %s" % browser, 3)
            return "Failed"
    except Exception, e:
        print "Exception : ", e
        print "Unable to start WebDriver"
        CoreFrameWork.CommonUtil.ExecLog(sModuleInfo, "Unable to start WebDriver", 3)
...
def OpenLink(link):
    ...
    try:
        ...
        CoreFrameWork.CommonUtil.ExecLog(sModuleInfo, "Successfully opened your link: %s" % link, 1)
        print "Successfully opened your link: " + link
        CoreFrameWork.CommonUtil.TakeScreenShot("Demo")
        return "PASSED"
    except Exception, e:
        print "Exception : ", e
        CoreFrameWork.CommonUtil.ExecLog(sModuleInfo, "Failed to open your link: %s" % link, 3)
...
def SelectFirstLevelMenu(menu_name_1):
    ...
    try:
        ...
        if len(allElements) == 0:
            ...
            CoreFrameWork.CommonUtil.ExecLog(sModuleInfo, "Unable to find your main menu object: %s" % menu_name_1, 3)
            CoreFrameWork.CommonUtil.TakeScreenShot("Demo")
            return "Failed"
        for element in allElements:
            if menu_name_1 == element.text:
                CoreFrameWork.CommonUtil.ExecLog(sModuleInfo, "Found your main menu item: %s" % element.text, 1)
                print "Found the Main Menu: " + element.text
                result = HoeverOver(element)
                CoreFrameWork.CommonUtil.ExecLog(sModuleInfo, "Hovering over to your main menu: %s" % menu_name_1, 1)
                if result != "PASSED":
                    CoreFrameWork.CommonUtil.ExecLog(sModuleInfo, "Unable to hover over your main menu: %s" % menu_name_1, 3)
                    CoreFrameWork.CommonUtil.TakeScreenShot("Demo")
                    return "Failed"
                else:
                    CoreFrameWork.CommonUtil.ExecLog(sModuleInfo, "Successfully hovered over your main menu: %s" % menu_name_1, 1)
                    CoreFrameWork.CommonUtil.TakeScreenShot("Demo")
                    return "PASSED"
        ...
        print "Exception : ", e
        CoreFrameWork.CommonUtil.ExecLog(sModuleInfo, "Error trying to select the main menu: %s" % menu_name_1, 3)
...
def SelectSecondLevelMenu(menu_name_2):
    ...
    try:
        ...
        if len(allElements) == 0:
            ...
            CoreFrameWork.CommonUtil.ExecLog(sModuleInfo, "Unable to find your second level menu object: %s" % menu_name_2, 3)
            CoreFrameWork.CommonUtil.TakeScreenShot("Demo")
            return "Failed"
        for element in allElements:
            if menu_name_2 == element.text:
                print "Found the second level menu: " + element.text
                CoreFrameWork.CommonUtil.ExecLog(sModuleInfo, "Found the second level menu:  %s" % menu_name_2, 3)
                CoreFrameWork.CommonUtil.ExecLog(sModuleInfo, "Hovering over to your second level menu: %s" % menu_name_2, 1)
                result = HoeverOver(element)
                if result != "PASSED":
                    CoreFrameWork.CommonUtil.ExecLog(sModuleInfo, "Unable to hover over your second level element: %s" % menu_name_2, 3)
                    CoreFrameWork.CommonUtil.TakeScreenShot("Demo")
                    return "Failed"
                else:
                    CoreFrameWork.CommonUtil.ExecLog(sModuleInfo, "Successfully hovered over your second level menu: %s" % menu_name_2, 1)
                    CoreFrameWork.CommonUtil.TakeScreenShot("Demo")
                    return "PASSED"
        print "Unable to find your element: " + menu_name_2
        CoreFrameWork.CommonUtil.ExecLog(sModuleInfo, "Unable to find second level menu item: %s" % menu_name_2, 3)
        CoreFrameWork.CommonUtil.TakeScreenShot("Demo")
        return "Failed"
    except Exception, e:
        print "Exception : ", e
        CoreFrameWork.CommonUtil.ExecLog(sModuleInfo, "Error trying to select the second level menu: %s" % menu_name_2, 3)
    ...
    try:
        ...
        for eachElements in allElements:
            ...
            if  menu_name_3 == clean_text:
                ...
                sBrowser.get(eachElements.get_attribute('href'))
                print "Successfully clicked your item: " + menu_name_3
                CoreFrameWork.CommonUtil.ExecLog(sModuleInfo, "Successfully clicked your item: %s" % menu_name_3, 1)
                CoreFrameWork.CommonUtil.TakeScreenShot("Demo")
                return "PASSED"
        print "Unable to find your item to click: " + menu_name_3
        CoreFrameWork.CommonUtil.ExecLog(sModuleInfo, "Unable to find your item to click: %s" % menu_name_3, 3)
        CoreFrameWork.CommonUtil.TakeScreenShot("Demo")
        return "Failed"
    except Exception, e:
        print "Exception : ", e
        CoreFrameWork.CommonUtil.ExecLog(sModuleInfo, "Error trying to select the 3rd level menu: %s" % menu_name_3, 3)
        print "Error trying to select the 3rd level menu"
        CoreFrameWork.CommonUtil.TakeScreenShot("Demo")
        return "Failed"
...
def FilterBySelection(check_box_name):
    ...
    try:
        ...
        for elem in (WebDriverWait(sBrowser, 5).until(lambda driver: sBrowser.find_elements_by_xpath('.//li[@class="%s"]' % check_box))):
            ...
            CoreFrameWork.CommonUtil.ExecLog(sModuleInfo, "Your check box %s is found but it is currently not checked" % check_box_name, 3)
            CoreFrameWork.CommonUtil.TakeScreenShot("Demo")
            for elem in sBrowser.find_elements_by_xpath('.//span[@class = "item"]'):
                if elem.text == check_box_name:
                    ...
                    CoreFrameWork.CommonUtil.ExecLog(sModuleInfo, "Your check box: %s is now checked" % check_box_name, 3)
                    CoreFrameWork.CommonUtil.TakeScreenShot("Demo")
                    return "PASSED"
        '''
        ...
        for elem in sBrowser.find_elements_by_xpath('.//li[@class="%s"]' % if_check_box_checked):
            ...
            CoreFrameWork.CommonUtil.ExecLog(sModuleInfo, "Your check box %s was already selected." % check_box_name, 3)
            CoreFrameWork.CommonUtil.TakeScreenShot("Demo")
            return "PASSED"
        print "Unable to locate your check box: %s" % check_box_name
        CoreFrameWork.CommonUtil.ExecLog(sModuleInfo, "Unable to locate your check box: %s" % check_box_name, 3)
        CoreFrameWork.CommonUtil.TakeScreenShot("Demo")
        return "Failed"

    except Exception, e:
        print "Exception : ", e
        CoreFrameWork.CommonUtil.ExecLog(sModuleInfo, "Unable to select your check box: %s" % check_box_name, 3)
...
def CloseBrowser():
    ...
    try:
        ...
        print "Exception : ", e
        CoreFrameWork.CommonUtil.ExecLog(sModuleInfo, "No open browser to close", 3)
...
def SearchItem(search_text, search_box="Search products, articles & help topics", search_button_id="ctl00_MasterPageHeader_GlobalSearchUC_BtnSubmitSearch"):
    ...
    try:
        ...
        CoreFrameWork.CommonUtil.ExecLog(sModuleInfo, "Searching for: %s" % search_text, 1)
        for elem in sBrowser.find_elements_by_xpath('//input[@title="%s"]' % search_box):
            print "Found the search box"
            CoreFrameWork.CommonUtil.ExecLog(sModuleInfo, "Found the search box", 1)
            elem.send_keys(search_text)
            print "Entered search item %s in the box" % search_box
            CoreFrameWork.CommonUtil.TakeScreenShot("Demo")
            CoreFrameWork.CommonUtil.ExecLog(sModuleInfo, "Entered search item %s in the box" % search_box, 1)
            search_button = False
            ...
            if search_button != False:
                ...
                CoreFrameWork.CommonUtil.ExecLog(sModuleInfo, "Successfully clicked the search item", 1)
                try:
                    WebDriverWait(sBrowser, 30)
                except Exception, e:
                    print "Exception : ", e
                    CoreFrameWork.CommonUtil.ExecLog(sModuleInfo, "Timed out waiting over 30 seconds for the page to load", 3)
                    print "Timed out waiting over 30 seconds for the page to load"
                    CoreFrameWork.CommonUtil.TakeScreenShot("Demo")
                    return "Failed"
                print "Waited for browser to load"
                CoreFrameWork.CommonUtil.ExecLog(sModuleInfo, "Waited for browser to fully load: %s" % search_text, 1)
                CoreFrameWork.CommonUtil.TakeScreenShot("Demo")
                return "PASSED"
            else:
                print "Search button was not found"
                CoreFrameWork.CommonUtil.ExecLog(sModuleInfo, "Search button was not found: %s" % search_text, 3)
                CoreFrameWork.CommonUtil.TakeScreenShot("Demo")
                return "Failed"
        print "Unable to find search box"
        CoreFrameWork.CommonUtil.ExecLog(sModuleInfo, "Unable to find search box: %s" % search_text, 3)
        CoreFrameWork.CommonUtil.TakeScreenShot("Demo")
        return "Failed"
    except Exception, e:
        print "Exception : ", e
        CoreFrameWork.CommonUtil.ExecLog(sModuleInfo, "Framework error with searching your item: %s" % search_text, 3)
...
def GetItemDetail():
    ...
    #    details_specs = []
        ...
        print "Exception : ", e
        CoreFrameWork.CommonUtil.ExecLog(sModuleInfo, "Framework error with searching your item: %s" % e, 3)
        CoreFrameWork.CommonUtil.TakeScreenShot("Demo")
        return "Failed"
...
def getShoeCount(size):
    ...
    try:
        ...
        print "Exception : ", e
        CoreFrameWork.CommonUtil.ExecLog(sModuleInfo, "Framework error with getting shoe count: %s" % size, 3)
