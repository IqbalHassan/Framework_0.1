    ...
    try:
        ...
        if MenuOrWindow == None:
            ...
            for eachWind in MainItems:
                try:
                    if eachWind.attributes[u'AXIdentifier'].value.get() == 'mainWindow':#'_NS:6':
            ...
            parent_ID = app(u'System Events').processes[u'ProductName'].menu_bars[1].UI_elements.get()
...
def Find_Element(elem_id=None, elem_path=None, elem_role=None, elem_value=None, elem_position=None, MenuOrWindow=None):
    try:
        sModuleInfo = inspect.stack()[0][3] + " : " + inspect.getmoduleinfo(__file__).name
        Element = [Key for Key, Value in Global.Ids.items() if Value == elem_id or Value == elem_path]
        for N in Element:
            if "DeviceMenu" in N: MenuOrWindow = "Menu"
...
def wait_for_obj_to_appear(ElemId=None, ElemPath=None, ElemRole=None, ElemValue=None, ElemPosition=None, Timeout=15):
    try:
        sModuleInfo = inspect.stack()[0][3] + " : " + inspect.getmoduleinfo(__file__).name
...
#                CommonUtil.ExecLog(sModuleInfo,"Timeout: Element (%s) was not found in (%s) seconds!" %(ElementName,wait),3)
                return False
            element = Find_Element(elem_id=ElemId, elem_role=ElemRole, elem_value=ElemValue, elem_position=ElemPosition)
...
def click_given_loc_appscript(id_path):
    ...
    try:
        ...
        app(u'System Events').processes[u'ProductName'].click(at=[x_position, y_position])
...
def is_check_box_enable_disabled(ElemId=None, ElemPath=None, ElemRole=None, ElemValue=None, ElemPosition=None):
    sModuleInfo = inspect.stack()[0][3] + " : " + inspect.getmoduleinfo(__file__).name
    try:
        ElementName = [Key for Key, Value in Global.Ids.items() if Value == ElemId or Value == ElemPath]
        element = Find_Element(elem_id=ElemId, elem_path=ElemPath, elem_role=ElemRole, elem_value=ElemValue, elem_position=ElemPosition)
...
def click_given_loc(loc):
    ...
    try:
        ...
        os.system ('%s/MouseTools -x %s, -y %s' % (os.path.dirname(os.path.abspath(__file__)), x_position, y_position))
